    ...
    .exec((err, user) => {
      ...
      UserData.findOne(
        ...
        if (!datas) {
          ...
          res.status(200).send(parseUserdata(user, token, dataconfig.USERDETAILS));
        } else {
          res.status(200).send(parseUserdata(user, token, datas.data[0]));
        }
      }).catch(err => {
        console.log('error accured in add event');
        console.log(err);
    ...
    if (verified) {
      ...
      User.findOne({ _id: id })
        ...
        .exec((err, user) => {
          ...
          if (!user) {
            return res.status(561).send({ verified: false, msg: "user not found" });
          }
...
function parseUserdata(userr, token, datas) {
  var authorities = [];
  ...
  let userInfo = {
    ...
    email: userr.email,
    roles: authorities,
    configs: userr.configs,
    contacts: userr.contacts,
    token: token,
    ...
    info: { classesCount: userr.classes.length, classesenrollCount: userr.enrolledIn.length + userr.AcceptedIn.length , classesAcceptedInCount: userr.AcceptedIn.length },
