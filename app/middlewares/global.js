...
function addscheduleEvent  (key, type, time, data, scheduleId)  {
  try {
    schedule.findOne(
      { key: key }, function (err, schedulefound) {
        ...
        if (scheduleId == -1) {
          ...
          schedulefound.save((err, data) => { console.log(err); checkscheduleEvent(null); });
